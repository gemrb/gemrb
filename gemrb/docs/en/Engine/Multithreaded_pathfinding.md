...
**The traversability cache.** This is a per-searchmap-tile record of how many actors cover a cell and whether any of them cannot be bumped.
Each actor contributes a token - 1 when it is bumpable, 15 when it is not - and the cell holds their sum, so 0 means empty and 15 or more means a non-bumpable actor is present. At searchmap resolution it is one byte per tile, small enough to copy outright when a snapshot is taken.
