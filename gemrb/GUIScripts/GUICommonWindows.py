		...
		if window:
			...
			if optwin:
				...
				if btn:
					...
					optwin.SetVar("OPTBTN", val) # cant use btn.ID because it is "too large to convert to C long"
				if rtgbtn: # not in PST or IWD2
...
def OpenPortraitWindow (needcontrols=0, pos=WINDOW_RIGHT|WINDOW_VCENTER):
	...
	for Button in PortraitButtons:
		...
		if not GameCheck.IsPST():
			...
			label = Button.CreateLabel(200 + pcID, fontref, "", align) #level up icon is on the right
...
def UpdatePortraitWindow ():
	...
	def EnablePortrait(Button):
		Button.SetAction(lambda btn, pc: GemRB.GameControlLocateActor(pc), IE_ACT_MOUSE_ENTER);
	...
	for Button in PortraitButtons:
		...
		elif barter_pc != -1 and pcID != barter_pc and GemRB.GetView("WIN_STORE"):
