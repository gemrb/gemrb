// SPDX-FileCopyrightText: 2018 Contributors to the GemRB project <https://gemrb.org>
//
...
struct GEM_EXPORT PixelFormat {
	...
	PixelFormat(uint8_t rloss, uint8_t gloss, uint8_t bloss, uint8_t aloss,
		...
		: Rloss(rloss), Gloss(gloss), Bloss(bloss), Aloss(aloss), Rshift(rshift), Gshift(gshift), Bshift(bshift), Ashift(ashift), Rmask(rmask), Gmask(gmask), Bmask(bmask), Amask(amask), Bpp(bpp), Depth(depth), ColorKey(ck), HasColorKey(hasCk), RLE(rle), palette(std::move(pal))
	{}
	...
	{
		return PixelFormat {
			0, 0, 0, 0, 16, 8, 0, 24, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000, 4, 32, 0, false, false, {}
		};
	}
};
...
struct GEM_EXPORT StaticAlphaIterator : public IAlphaIterator {
	...
	explicit StaticAlphaIterator(uint8_t a)
		: alpha(a) {}
