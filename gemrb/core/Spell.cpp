...
void Spell::AddCastingGlow(EffectQueue* fxqueue, ieDword duration, int gender) const
{
	...
	if (cgsound >= 0 && duration > 1) {
		...
		caster->castingSound = core->GetAudioPlayback().PlayDirectional(Resource, SFXChannel::Casting, caster->Pos, caster->GetOrientation(), caster->GetCurrentArea());
	}
	...
	for (size_t i = 0; i < count; ++i) {
		...
		if (caster) {
			...
			if (SpellType == IE_SPL_WIZARD) {
				factor = caster->Modified[IE_SPELLDURATIONMODMAGE];
			} else if (SpellType == IE_SPL_PRIEST) {
				factor = caster->Modified[IE_SPELLDURATIONMODPRIEST];
				...
				factor = durFx ? durFx->Parameter1 : 0;
			}
			if (factor) {
