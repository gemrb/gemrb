...
bool DialogHandler::InitDialog(Scriptable* spk, Scriptable* tgt, const ResRef& dialogRef, ieDword si)
{
	...
	if (!dlg) {
		Log(ERROR, "DialogHandler", "Cannot start dialog (%s): %ls with %ls", dialogRef.CString(), spk->GetName().c_str(), tgt->GetName().c_str());
		return false;
	}
...
bool DialogHandler::DialogChoose(unsigned int choose)
{
	...
	if (tgta) {
		...
		ta->AppendText(L"\n");
		displaymsg->DisplayStringName( ds->StrRef, DMC_DIALOG, target, STRING_FLAGS::SOUND | STRING_FLAGS::SPEECH);
	}
