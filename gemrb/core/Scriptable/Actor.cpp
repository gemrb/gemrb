	...
	if (iwd2class) {
		...
		*/
		...
		for (i=0; i<classcount; i++) {
			...
			classnames[0] = (char*)strtok(strdup(const_cast<char*>(classname)), "_");
...
void Actor::VerbalConstant(int start, int count) const
{
	...
	if (PCStats && PCStats->SoundSet[0]) {
		...
		if (count > 0){
			DisplayStringCore(const_cast<Scriptable *const>(dynamic_cast<const Scriptable *const>(this)), start + RAND(0, count-1), DS_CONSOLE|DS_CONST|DS_SPEECH);
		}
		...
		if(count > 0) {
			DisplayStringCore(const_cast<Scriptable *const>(dynamic_cast<const Scriptable *const>(this)), GetVerbalConstant(start+RAND(0, count-1)), DS_CONSOLE|DS_SPEECH);
		}
	}
}
...
void Actor::DisplayStringOrVerbalConstant(int str, int vcstat, int vccount) const {
	...
	if (strref != -1) {
		DisplayStringCore(const_cast<Scriptable *const>(dynamic_cast<const Scriptable *const>(this)), strref, DS_CONSOLE);
	} else {
