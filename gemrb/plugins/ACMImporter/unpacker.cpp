...
FillerProc Fillers[32] = {
	...
	&CValueUnpacker::k1_2bits,
	&CValueUnpacker::t1_5bits,
	&CValueUnpacker::k2_4bits,
	&CValueUnpacker::k2_3bits,
	&CValueUnpacker::t2_7bits,
	&CValueUnpacker::k3_5bits,
	&CValueUnpacker::k3_4bits,
	&CValueUnpacker::return0,
	&CValueUnpacker::k4_5bits,
...
//Rus: имеет смысл использовать, когда веро€тность парных нулей (p00) больше 2/3
	for (int i=0; i<subblocks; i++) {
		...
		if ((next_bits & 1) == 0) {
			avail_bits--;
			next_bits >>= 1;
			block_ptr [i*sb_size + pass] = 0; if ((++i) == subblocks) break;
			...
			block_ptr [i*sb_size + pass] = (next_bits & 4)? buff_middle[1]: buff_middle[-1];
			avail_bits -= 3;
			next_bits >>= 3;
		}
	}
...
//Rus: имеет смысл использовать, когда веро€тность нул€ больше 1/3
	for (int i=0; i<subblocks; i++) {
		...
		if ((next_bits & 1) == 0) {
			...
			block_ptr [i*sb_size + pass] = (next_bits & 2)? buff_middle[1]: buff_middle[-1];
			avail_bits -= 2;
			next_bits >>= 2;
		}
	}
...
// can be used when frequency of paired zeros (p00) is greater than 2/3
	for (int i=0; i<subblocks; i++) {
		...
		if ((next_bits & 1) == 0) {
			...
			block_ptr [i*sb_size + pass] = (next_bits & 8)? buff_middle[1]: buff_middle[-1];
			avail_bits -= 4;
			next_bits >>= 4;
		} else {
			avail_bits -= 5;
