...
Actor* CREImporter::GetActor(unsigned char is_in_party)
{
	...
	if (core->HasFeature(GFFlags::HAS_EE_EFFECTS)) {
		if (!act->SmallPortrait.IsEmpty() && act->SmallPortrait != "NONE") {
...
int CREImporter::PutHeader(DataStream* stream, const Actor* actor) const
{
	...
	if (core->HasFeature(GFFlags::HAS_EE_EFFECTS) && *actor->LargePortrait.rbegin() != 'M' && actor->SmallPortrait != "NONE") {
	...
	for (int i = 0; i < type; i++) {
		...
		for (unsigned int j = 0; j < level; j++) {
			...
			for (int k = knownCount - 1; k >= 0; k--) {
				auto ck = actor->spellbook.GetKnownSpell(i, j, k);
				assert(ck);
...
int CREImporter::PutSpellPages(DataStream* stream, const Actor* actor) const
{
	...
	for (int i = 0; i < type; i++) {
		...
		for (ieWord j = 0; j < level; ++j) {
			...
			if (actor->creVersion == CREVersion::V1_1 || actor->creVersion == CREVersion::V1_2 || (actor->creVersion == CREVersion::V9_0 && core->config.GameType == "how")) {
				// pst didn't use the table, even though it kept a copy from bg1
				// iwd1:how didn't store the bonus, iwd1 did??
				// bg2ee also appears to skip the wisdom bonus
...
int CREImporter::PutIWD2Spellpage(DataStream* stream, const Actor* actor, ieIWD2SpellType type, int level) const
{
	...
	for (int i = 0; i < knownCount; i++) {
		auto knownSpell = actor->spellbook.GetKnownSpell(type, level, i);
