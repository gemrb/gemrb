...
bool VLCPlayer::Open(DataStream* stream)
{
	...
	if (stream) {
		...
		Log(DEBUG, "VLC Player", "assigning video callbacks with context pointerat address:%X", &ctx);
		...
		if (ret == GEM_OK) {
			...
			while (!done && libvlc_media_player_is_playing(mediaPlayer)) {
				...
				if (ctx->isYUV()) {
					...
					video->showYUVFrame((unsigned char**)planes, strides, ctx->Width(), ctx->Height(), ctx->Width(), ctx->Height(), 0, 0, 0);
				} else {
					video->showFrame((unsigned char*)ctx->GetPlane(0), ctx->Width(), ctx->Height(), 0, 0, ctx->Width(), ctx->Height(), 0, 0, true, NULL, 0);
				}
				ctx->Unlock();
			}
		}
