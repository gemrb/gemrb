...
int fx_set_poisoned_state (Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	case RPD_SNAKE: //iwd2 snakebite (a poison causing paralysis)
		...
		if (fx->FirstApply) {
			displaymsg->DisplayConstantStringName(STR_HELD, colorsType::WHITE, target);
		}
		break;
	case RPD_7:
		damage = fx->Parameter1;
	...
	if (!fx->Parameter2) {
		if (target->inventory.GetItemFlag(slot)&IE_ITEM_TWO_HANDED) {
			if (target->inventory.HasItemInSlot("",target->inventory.GetShieldSlot())) {
				...
				displaymsg->DisplayConstantStringNameString(STR_SPELL_FAILED, colorsType::WHITE, STR_OFFHAND_USED, target);
				return FX_NOT_APPLIED;
			}
		}
	}
...
int fx_display_string (Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	if (!target->fxqueue.HasEffectWithParamPair(fx_protection_from_display_string_ref, fx->Parameter1, 0) ) {
		displaymsg->DisplayStringName(ieStrRef(fx->Parameter1), colorsType::WHITE, target, STRING_FLAGS::SOUND | STRING_FLAGS::SPEECH);
	}
	return FX_NOT_APPLIED;
}
...
int fx_disable_spellcasting (Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	if (fx->FirstApply && target->GetStat(IE_EA) < EA_CONTROLLABLE) {
		if (display_warning) displaymsg->DisplayConstantStringName(STR_DISABLEDMAGE, colorsType::RED, target);
		core->SetEventFlag(EF_ACTION);
	}
	return FX_APPLIED;
}
...
int fx_find_familiar (Scriptable* Owner, Actor* target, Effect* fx)
{
	...
	if (game->familiarBlock) {
		displaymsg->DisplayConstantStringName(STR_FAMBLOCK, colorsType::RED, target);
		return FX_NOT_APPLIED;
	}

	//The protagonist is ALWAYS in the first slot
	if (game->GetPC(0, false)!=target) {
		displaymsg->DisplayConstantStringName(STR_FAMPROTAGONIST, colorsType::RED, target);
		return FX_NOT_APPLIED;
	}
	...
	if (condition) {
		...
		for (unsigned int i = 0; i < 4; i++) {
			...
			if (fx->Parameter2 == COND_GOTHIT) {
				...
				if (!dist) {
					displaymsg->DisplayConstantStringName(STR_CONTFAIL, colorsType::RED, target);
					continue;
				}
...
int fx_create_contingency (Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	if (target->fxqueue.HasEffectWithSource(fx_contingency_ref, fx->SourceRef)) {
		displaymsg->DisplayConstantStringName(STR_CONTDUP, colorsType::WHITE, target);
		return FX_NOT_APPLIED;
	}
...
int fx_set_area_effect (Scriptable* Owner, Actor* target, Effect* fx)
{
	...
	if (map->GetTrapCount(iter) + 1 > gamedata->GetTrapLimit(Owner)) {
		displaymsg->DisplayConstantStringName(STR_NOMORETRAP, colorsType::WHITE, target);
		return FX_NOT_APPLIED;
	}

	//check if we are under attack
	if (GetNearestEnemyOf(map, target, ORIGIN_SEES_ENEMY|ENEMY_SEES_ORIGIN)) {
		displaymsg->DisplayConstantStringName(STR_MAYNOTSETTRAP, colorsType::WHITE, target);
		return FX_NOT_APPLIED;
	}
	...
	if (roll>skill) {
		//failure
		displaymsg->DisplayConstantStringName(STR_SNAREFAILED, colorsType::WHITE, target);
	...
	//success
	displaymsg->DisplayConstantStringName(STR_SNARESUCCEED, colorsType::WHITE, target);
...
int fx_create_spell_sequencer(Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	if (target->fxqueue.HasEffectWithSource(fx_spell_sequencer_active_ref, fx->SourceRef)) {
		displaymsg->DisplayConstantStringName(STR_SEQDUP, colorsType::WHITE, target);
		return FX_NOT_APPLIED;
	}
...
int fx_resist_spell_and_message (Scriptable* Owner, Actor* target, Effect *fx)
{
	...
	if (sourceNameRef != ieStrRef(-1)) {
		...
		displaymsg->DisplayConstantStringName(STR_RES_RESISTED, colorsType::WHITE, target);
	}
	//this has effect only on first apply, it will stop applying the spell
	return FX_ABORT;
}
