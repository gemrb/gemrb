...
mve_cb_CreateAudioStream mve_createaudiostream;
mve_cb_FreeAudioStream mve_freeaudiostream;
mve_cb_QueueBuffer mve_queuebuffer;
...
void mvefile_advance_segment(MVEFILE* movie)
{
	...
	movie->next_segment += ( 4 + _mve_get_ushort( movie->cur_chunk +
									movie->next_segment ) );
}
 ...
 */
void mve_set_handler(MVESTREAM* movie, unsigned char major,
	MVESEGMENTHANDLER handler)
{
	if (major < 32) {
