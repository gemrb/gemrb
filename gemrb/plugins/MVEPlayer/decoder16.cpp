...
static void dispatchDecoder16(unsigned short** pFrame, unsigned char codeType,
	unsigned char** pData, unsigned char** pOffData, int* pDataRemain,
	int* curXb, int* curYb);
	...
	unsigned char* pData, int dataRemain)
{
	...
	for (j = 0; j < yb; j++) {
		for (i = 0; i < xb / 2; i++) {
			op = ( *pMap ) & 0xf;
			dispatchDecoder16( &FramePtr, op, &pData, &pOffData, &dataRemain,
				&i, &j );
...
static void patternRow4Pixels(unsigned short* pFrame, unsigned char pat0,
...
static void patternRow4Pixels2(unsigned short* pFrame, unsigned char pat0,
	unsigned short* p)
{
...
static void patternQuadrant4Pixels(unsigned short* pFrame, unsigned char pat0,
