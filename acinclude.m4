...
dnl AM_PATH_OPENAL([MINIMUM-VERSION, [ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND]]])
...
AC_ARG_ENABLE(openaltest, [  --disable-openaltest       Do not try to compile and run a test OPENAL program],
  ...
  min_openal_version=ifelse([$1], ,0.11.0,$1)
  AC_MSG_CHECKING(for OPENAL - version >= $min_openal_version)
  no_openal=""
  if test "$OPENAL_CONFIG" = "no" ; then
    ...
    openal_major_version=`$OPENAL_CONFIG $openal_args --version | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\1/'`
    openal_minor_version=`$OPENAL_CONFIG $openal_args --version | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\2/'`
    openal_micro_version=`$OPENAL_CONFIG $openal_config_args --version | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\3/'`
...
dnl
dnl Now check if the installed OPENAL is sufficiently new. (Also sanity
dnl checks the results of openal-config to some extent
dnl
...
int main (int argc, char *argv[])
{
  ...
  */
  { FILE *fp = fopen("conf.openaltest", "a"); if ( fp ) fclose(fp); }

  /* HP/UX 9 (%@#!) writes to sscanf strings */
  tmp_version = my_strdup("$min_openal_version");
  if (sscanf(tmp_version, "%d.%d.%d", &major, &minor, &micro) != 3) {
     printf("%s, bad version string\n", "$min_openal_version");
     exit(1);
   }

   if (($openal_major_version > major) ||
      (($openal_major_version == major) && ($openal_minor_version > minor)) ||
      (($openal_major_version == major) && ($openal_minor_version == minor) && ($openal_micro_version >= micro)))
    {
      return 0;
    }
  else
    {
      printf("\n*** 'openal-config --version' returned %d.%d.%d, but the minimum version\n", $openal_major_version, $openal_minor_version, $openal_micro_version);
      printf("*** of OPENAL required is %d.%d.%d. If openal-config is correct, then it is\n", major, minor, micro);
      printf("*** best to upgrade to the required version.\n");
  ...
  if test "x$no_openal" = x ; then
     ...
     if test "$OPENAL_CONFIG" = "no" ; then
       ...
       echo "*** If OPENAL was installed in PREFIX, make sure PREFIX/bin is in"
       echo "*** your path, or set the OPENAL_CONFIG environment variable to the"
       echo "*** full path to openal-config."
     else
...
#define main K_and_R_C_main
	  ...
	  echo "***"
        ...
        [ echo "*** The test program failed to compile or link. See the file config.log for the"
          ...
          echo "*** or that you have moved OPENAL since it was installed. In the latter case, you"
          echo "*** may want to edit the openal-config script: $OPENAL_CONFIG" ])
          CFLAGS="$ac_save_CFLAGS"
          LIBS="$ac_save_LIBS"
       fi
     fi
